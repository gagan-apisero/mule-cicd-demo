				...
				script{
			        if("SUCCESS".equals(currentBuild.previousBuild.result)){
                        if(env.GIT_BRANCH == "dev")  {
                            ...
                            bat 'mvn deploy -DmuleDeploy -Danypoint.username=%ANYPOINT_CREDENTIALS_USR% -Danypoint.password=%ANYPOINT_CREDENTIALS_PSW% -Danypoint.platform.client_id=%ANYPOINT_CLIENT_ID% -Danypoint.platform.client_secret=%ANYPOINT_CLIENT_SECRET% -Danypoint.env=Sandbox -Danypoint.region=us-east-1 -Danypoint.workers=1 -Danypoint.name=%APP_NAME%-dev -Dmule.artifact=%WORKSPACE%\\target\\%APP%-mule-application.jar'
                        }
                            ...
                            timeout(time: 30, unit: 'MINUTES'){
                                input "Deploy to QA?" }
                                bat 'mvn deploy -DmuleDeploy -Danypoint.username=%ANYPOINT_CREDENTIALS_USR% -Danypoint.password=%ANYPOINT_CREDENTIALS_PSW% -Danypoint.platform.client_id=%ANYPOINT_CLIENT_ID% -Danypoint.platform.client_secret=%ANYPOINT_CLIENT_SECRET% -Danypoint.env=Sandbox -Danypoint.region=us-east-1 -Danypoint.workers=1 -Danypoint.name=%APP_NAME%-qa -Dmule.artifact=%WORKSPACE%\\target\\%APP%-mule-application.jar'
                        }
