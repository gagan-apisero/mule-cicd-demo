  ...
  stages {
    stage('Build Application') {
			steps {
				...
				bat 'mvn clean install -Djar.name=%APP_NAME%'
			...
			steps {
				script{
                        if(env.GIT_BRANCH == "dev")  {
                            ...
                            bat 'mvn deploy -DmuleDeploy -Danypoint.username=%ANYPOINT_CREDENTIALS_USR% -Danypoint.password=%ANYPOINT_CREDENTIALS_PSW% -Danypoint.platform.client_id=%ANYPOINT_CLIENT_ID% -Danypoint.platform.client_secret=%ANYPOINT_CLIENT_SECRET% -Danypoint.env=Sandbox -Danypoint.region=us-east-1 -Danypoint.workers=1 -Danypoint.name=%APP_NAME%-dev -Djar.name=%APP_NAME%'
                        }
